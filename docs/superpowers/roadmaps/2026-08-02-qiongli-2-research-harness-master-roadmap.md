...
| **NEXT** | Deliver CLI → Plugin/MCP → Host onboarding: first align commands, versions and documentation and add Cargo source distribution (ADR 0221); then resolve registry CLI Plugin lifecycle through existing approval/CAS owners; finally qualify real Host discovery, project read, handoff and approved write. Complete baseline acceptance remains separate. |
