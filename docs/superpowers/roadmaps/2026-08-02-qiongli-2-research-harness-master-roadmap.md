...
| **NEXT** | `CLI-410` alpha.6 is published as a standalone CLI GitHub Release under ADR 0219; macOS ARM64 executable and npm/wheel downloads passed independent install verification. Follow up on Cargo/npm/PyPI registry uploads, managed Plugin/install qualification and remaining baseline evidence. App/Community Alpha promotion and offline signing keys do not gate direct CLI downloads; research approval/CAS checks remain unchanged. Windows installation remains partially complete/paused. Rich Graph v1 belongs to `PLT-322` in M2. |
