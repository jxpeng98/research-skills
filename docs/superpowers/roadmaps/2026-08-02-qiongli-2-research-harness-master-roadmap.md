...
| **NEXT** | Codex-first Skills quality: shared instruction consistency is implemented at `6960ccc4`; the 48-request bilingual intent corpus and Codex capture/score path are implemented at `70a0fe6a`, with a remaining-work oracle correction at `d6fcad27`. Twelve real supplied-entry requests were sampled (original labels 10/12); the two corrected-oracle cases passed a separate 2/2 run. Finish the other 36, then controlled version comparisons and actual Codex activation/resource/tool evidence; then source anchors, claim support and revision-aware continuation on one research journey. Intent labels are not observed activation or research quality. Other Agents adapt shared contracts. Local Plugin lifecycle (ADR 0222) and Cargo wiring (ADR 0221) remain implemented. Host registration stays paused; Cargo still needs named-release three-platform and registry checks. Acceptance stays separate. |
