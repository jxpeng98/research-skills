...
| **NOW** | `CLI-405` bounded same-device read-only handoff from Codex to Claude Code with configured DeepSeek is verified at candidate `dfebb17c`, revision 2. The alpha.6 Cargo/npm/PyPI local packaging and install increment is implemented with 50 native/real-client checks passed; signed lifecycle and public registry qualification remain open. Reconcile remaining declared baseline evidence; a complete academic graph is not required. Windows `CLI-403` native CLI package qualification is resumed; historical managed/App installation evidence remains partial. `CLI-401/402` source integration is merged; program acceptance remains separate. |
| **NEXT** | `CLI-410` alpha.7 standardizes SemVer/PEP 440 mapping, resumes native Windows/Linux installation alongside macOS, and publishes through existing npm `next` / PyPI channels after three-target and combined-package CI (ADR 0220). Cargo registry work is deferred. Alpha.6 remains the immutable macOS-only GitHub release. Managed Plugin/install and baseline acceptance remain separate; research approval/CAS is unchanged. |
