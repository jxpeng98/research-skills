...
| **NEXT** | Codex-first Skills quality: v2 separates task route, prerequisite resource, scope and current action; all 48 request texts are preserved. Source-bound offline regrading retains original scores and records post-hoc adjudications separately. An 18-answer no-Skill / preceding / candidate comparison gives common scope/action 6/6 per arm; both entry arms retain route/resource 4/6 because denied-read task-versus-stop labels remain ambiguous. Candidate answers avoid the unsupported measurement wording seen in the other English interpretations; this small sample establishes no general superiority. Next resolve that route interpretation from its owners, then measure actual Codex resource reads and the evidence/continuity journey in an isolated synthetic workspace. Other Agents adapt shared contracts. Host registration stays paused; Cargo still needs named-release three-platform and registry checks. Acceptance stays separate. |
